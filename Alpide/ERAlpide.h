...
namespace AlpideSpecs {
  ...
  const Double_t TargetThickness = 5.55;
}

namespace C7ExperimentDistances {
  const Double_t WindowToAlpide = 2.05; //cm
  const Double_t TargetCenterToWindow = 5.55 / 2; //cm
  const Double_t AlpideToTargetCenter = WindowToAlpide + TargetCenterToWindow; //cm
  const Double_t BetweenFootPair = 11.1;
  const Double_t FirstFootToAlpide = 12.5;
}
