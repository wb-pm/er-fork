...
{
  LOG(INFO) << "[ERFootMuSiVertexFinder]------------Started--------------------------------------"
    << FairLogger::endl;
  // Get input array
...
void ERFootMuSiVertexFinder::Exec(Option_t* opt)
{
  ...
  for (Int_t iTrack = 0; iTrack < fFootMuSiTracks->GetEntriesFast(); iTrack++) {
    ERFootMuSiTrack* firstTrack = (ERFootMuSiTrack*)fFootMuSiTracks->At(iTrack);
    for (Int_t jTrack = 1; jTrack < fFootMuSiTracks->GetEntriesFast(); jTrack++) {
      ERFootMuSiTrack* secondTrack = (ERFootMuSiTrack*)fFootMuSiTracks->At(jTrack);
      for (Int_t kTrack = 2; kTrack < fFootMuSiTracks->GetEntriesFast(); kTrack++) {
        if ((iTrack == jTrack) || (iTrack == kTrack) || (jTrack == kTrack))
        {
          continue;
        }
        ERFootMuSiTrack* thirdTrack = (ERFootMuSiTrack*)fFootMuSiTracks->At(kTrack);
        //TODO: Get the distance between two strips from geometry
        //distance between two strips is 0.01667 cm = 1.6e-2 cm
        std::vector<ERFootMuSiTrack*> tempTracksVector = { firstTrack,secondTrack,thirdTrack };
        auto compareTracks = [](const ERFootMuSiTrack* trackNo1, const ERFootMuSiTrack* trackNo2, bool compareX) -> double {
          auto getCoordinate = [&](const TVector3& difference) -> double {
            if (compareX) {
              return difference.X();
            }
            else {
          ...
          double deltaFirstHit = std::fabs(getCoordinate(trackNo1->GetFirstHit() - trackNo2->GetFirstHit()));
          double deltaSecondHit = std::fabs(getCoordinate(trackNo1->GetSecondHit() - trackNo2->GetSecondHit()));
          double deltaThirdHit = std::fabs(getCoordinate(trackNo1->GetThirdHit() - trackNo2->GetThirdHit()));
          return std::min({ deltaFirstHit,deltaSecondHit,deltaThirdHit });
          };
        double deltaX12 = compareTracks(firstTrack, secondTrack, true);
        double deltaX13 = compareTracks(firstTrack, thirdTrack, true);
        double deltaX23 = compareTracks(secondTrack, thirdTrack, true);
        double deltaY12 = compareTracks(firstTrack, secondTrack, false);
        double deltaY13 = compareTracks(firstTrack, thirdTrack, false);
        double deltaY23 = compareTracks(secondTrack, thirdTrack, false);
        if (std::min({ deltaX12,deltaX13,deltaX23,deltaY12,deltaY13,deltaY23 }) > 0.016)
        {
          splitTracks.push_back(tempTracksVector);
        }
        else {
          continue;
        }
  ...
  LOG(INFO) << "number of vectors of track triples in the event: " << splitTracks.size() << FairLogger::endl;
  //Searching for a common perpendicular for a pair of tracks
  for (Int_t nVector = 0; nVector < splitTracks.size(); nVector++)
  {
    for (Int_t iTrack = 0; iTrack < splitTracks.at(nVector).size(); iTrack++) {
      ERFootMuSiTrack* track1 = splitTracks.at(nVector).at(iTrack);
      ...
      for (Int_t jTrack = iTrack + 1; jTrack < splitTracks.at(nVector).size(); jTrack++) {
        if (iTrack == jTrack)
          continue;
        ERFootMuSiTrack* track2 = splitTracks.at(nVector).at(jTrack);
...
{
  LOG(INFO) << "[ERFootMuSiVertexFinder]------------Finished--------------------------------------"
    << FairLogger::endl;
}
//----------------------------------------------------------------------------
